export const typesAlmacen = {
  setCargando: 'SET_CARGANDO ALMACEN',
  setNombre: '[SET] Nombre del Almacen',
  setCodigo: '[SET] Codigo del Almacen',
  setUbicacion: '[SET] Ubicacion del Almacen',
  setAlmacen: '[SET] Almacen',
  setClear: '[SET] Clear',
};

export const typesProveedor = {
  setCargando: 'SET_CARGANDO PROVEEDOR',
  setRazonSocial: '[SET] Razon Social',
  setDirecFiscal: '[SET] Direccion Fiscal',
  setRuc: '[SET] Ruc',
  setTipo: '[SET] Tipo',
  setCelular: '[SET] Celular',
  setCorreo: '[SET] Correo',
  setProveedor: '[SET] Proveedor',
  setClear: '[SET] Clear',
}

export const typesProducto = {
  setCodigo: '[SET] Código del Producto',
  setDescripcion: '[SET] Descripción del Producto',
  setAtributosId: '[SET] Atributos del Producto',
  setAlmacenId: '[SET] Almacen del Producto',
  setProducto: '[SET] Producto',
  setClear: '[SET] Clear',
};

